...
> **实现选择**：经典做法（Gatys/Johnson）多取更深的 `relu4_3`/`conv4_2`，本实现选 `relu3_3` 是在"保留更多空间结构"与"语义抽象"之间的折中——较浅的层保留更清晰的轮廓，配合本实验偏重内容保真的目标（见第 4 章 500:1 的选择）。代码用 `nn.MSELoss`，故含 $1/CHW$ 的归一化。
...
| 测试图像 | 2 张 | 自然场景照片 | 评估效果（test.jpg / test1.JPG） |
...
# 损失权重（关键：决定内容与风格的平衡）
content_weight = 10.0
style_weight = 5000    # content:style = 1:500（推荐），扫描范围 250:1 ~ 1000:1
tv_weight = 1e-4
```
...
1. **构建数据集与 DataLoader**：遍历 `data/train_images` 下 6912 张图，每张经 `Resize(512) → CenterCrop(512) → ToTensor ×255` 处理为 $[0,255]$ 的 $3\times512\times512$ 张量；`batch_size=16`、`shuffle=True`、`num_workers=4`。
...
每步同时将四个损失写入 CSV 日志（`logs/*.csv`），用于事后绘制收敛曲线（见第 4 章）。
...
> 本章按真实的探索时间顺序记录。最终的成品参数（content:style = 500:1, 16 epoch, 512 分辨率）
...
| ③ 基准 2ep | best_model | 1:1e5, 2ep, 256 | 85.4 | 风格初现，但内容模糊、有噪点 |
...
完整逐 epoch 曲线见训练日志 `logs/ratio_{250to1,500to1,1000to1}.csv` 与 `logs/ratio_sweep.out`。

### 4.3 比例扫描结果与最佳参数

三组在两张测试图（512→672 分辨率）上评估，得到**单调、可解释**的内容-风格权衡曲线：

| 比例 | 最优epoch | content_loss (test/test1) | style_loss (test/test1) | 推理(s) | 视觉特征 |
|------|----------|---------------------------|-------------------------|---------|---------|
| 250:1 | ep8 | **13143 / 13770**（最低） | 1065 / 1086 | 0.09 | 内容最清晰，风格最克制 |
| **500:1** ⭐ | ep6 | 15337 / 16282 | 1023 / 1004 | 0.09 | 内容与风格平衡（推荐） |
...
|------|:---:|:---:|:---:|:---:|:---:|------|
| 250:1 | 5 | 2 | 4 | 4 | 适合写实 | 保留照片细节优先 |
| **500:1** ⭐ | 4 | 4 | 4 | 4 | **均衡最佳** | 通用默认 |
...
综合量化指标与视觉效果，推荐 **content:style = 500:1**（`checkpoints/experiments/ratio_500to1.pth`）作为成品默认配置：

- 它位于权衡曲线的中点，内容清晰度与风格强度兼顾；
- 与早期 4.1 阶段⑨ 人工挑选的 `model_balanced_500` 配置一致，在严谨、收敛、best-checkpoint 的条件下**再次得到验证**；
- 250:1 风格偏弱；1000:1 内容细节损失偏多，且仅 3 个 epoch 就到最优、稳定裕度小。

若追求更强艺术感可选 1000:1，追求最大化保留照片细节可选 250:1。

#### 成品化：一键训练新风格

最佳配置已固化为参数化脚本 `scripts/train_style.sh`，更换风格图即可训练新模型：

```bash
# 用法：bash scripts/train_style.sh <风格图> <输出名> [比例,默认500]
bash scripts/train_style.sh data/style_images/vangogh.jpg vangogh
bash scripts/train_style.sh data/style_images/ink.jpg ink 250   # 更弱的风格
```
...
- 比例过小:风格不明显,接近原图;
- **平衡点**:经比例扫描,**content:style ≈ 500:1**(content_weight=10, style_weight=5000)在内容清晰度与风格强度之间取得最佳平衡。
...
| 数据准备 | ✅ | 整理 Natural Images 训练集（6912 张），准备风格图与 2 张测试图 |
...
> 说明：本实验的所有模型权重、训练日志、输出图像均由本人在上述环境中实际产生，可在 `checkpoints/`、`logs/`、`results/` 中复核；未使用任何他人预训练的风格迁移成品模型。
...
3. ✅ **参数摸索**：训练了 10+ 个模型，从失衡的 1:100000 比例逐步收敛到 **content:style = 500:1** 的平衡配置
4. ✅ **严谨验证**：通过固定配置、只扫合理比例区间的扫描实验，得到单调可解释的内容-风格权衡曲线
5. ✅ **成品化**：封装 `train_style.sh`，支持一键训练新风格模型

### 关键发现
- **比例 > 绝对值**：决定效果的是 content:style 比例，而非 style_weight 单独的数值；500:1 为本数据集/风格图的平衡点
...
│   ├── style_images/       # 风格图像
│   └── test_images/        # 测试图像 (test.jpg / test1.JPG)
├── checkpoints/
│   └── experiments/        # 比例扫描模型 ratio_{250to1,500to1,1000to1}.pth
├── logs/                   # 训练日志 ratio_*.csv
├── results/
│   └── experiments/        # 扫描输出 + ratio_sweep.csv + ratio_sweep_comparison.png
├── doc/  study/            # 参考资料与学习笔记
└── archive/                # 归档的过程文档
```
...
# 单独测试某个模型
python3 test.py --model checkpoints/experiments/ratio_500to1.pth \
